      ...
      <table id="table" class="table table-sm table-striped">
        <thead>
          <tr class="d-flex">
            ...
            <th scope="col" class="col-2">Flowering Period</th>
            <th scope="col" class="col-2">Chemical Composition</th>
            <th scope="col" class="col-2">Therapeutic Uses</th>
            <th scope="col" class="col-2">Image Link</th>
          </tr>
          <thead>
          <tbody>
            <?php 
        ...
        $sql = "SELECT * FROM `book1-Medicinal_plants_in_Viet_Nam` WHERE Plant_Name LIKE '$selected_val'";
        $result = mysqli_query($conn,$sql);
        while($row = mysqli_fetch_assoc($result)){
            echo "<tr class='d-flex'>
                    <td class='col-2'>{$row['Plant_Name']}</td>
                    <td class='col-2'>{$row['Local_Names']}</td>
                    <td class='col-2'>{$row['English_Names']}</td>
                    <td class='col-2'>{$row['Description']}</td>
                    <td class='col-2'>{$row['Distribution']}</td>
                    <td class='col-2'>{$row['Parts_Used']}</td>
                    <td class='col-2'>{$row['Flowering_Period']}</td>
                    <td class='col-2'>{$row['Chemical_Composition']}</td>
                    <td class='col-2'>{$row['Therapeutic_Uses']}</td>
                    <td class='col-2'>
