...
if(isset($_POST['submit'])){
    $fields = array('Plant_Name','English_Name','Korean_Name','Parts_Used','Traditional_Uses','Description','Habitat','Distribution','Bio-Activities','Chemical_Components');
    $conditions = array();
    ...
    // builds the query 
    $query = "SELECT * FROM `book2-Medicinal_Plants_of_Korea` ";
...
<table id="table" class="table table-sm table-striped">
 <thead>
  <tr>
      ...
      <th scope="col">Traditional Uses</th>
      <th scope="col">Description</th>
      <th scope="col">Habitat</th>
      <th scope="col">Distribution</th>
      <th scope="col">Bio-Activities</th>
      <th scope="col">Chemical Components</th>
...
while($row = mysqli_fetch_assoc($result)){
  echo "<tr>
      ...
      <td>{$row['Traditional_Uses']}</td>
      <td>{$row['Description']}</td>
      <td>{$row['Habitat']}</td>
      <td>{$row['Distribution']}</td>
      <td>{$row['Bio-Activities']}</td>
      <td>{$row['Chemical_Components']}</td>
      <td>
